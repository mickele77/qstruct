...
void TimberConnector::initVar( double ffyk, double ffuk, double dd ){
    ...
    fyk->setToolTip( tr("Tensione caratteristica di snervamento del connettore"));
    addVarToContainer( fyk );

    fuk = new DoublePlus( ffuk, "fuk", m_unitMeasure, UnitMeasure::tension );
    fuk->setReadOnly( false );
    fuk->setRichName( tr("f<span style=\" vertical-align:sub;\">uk</span>"));
    fuk->setToolTip( tr("Tensione caratteristica di rottura del connettore"));
    addVarToContainer( fuk );

    d = new DoublePlus( dd, "d", m_unitMeasure, UnitMeasure::sectL );
    d->setReadOnly( false );
    d->setRichName( tr("d"));
    d->setToolTip( tr("Diametro del connettore"));
    addVarToContainer( d );

    dEf = new DoublePlus( 0.0, "dEf", m_unitMeasure, UnitMeasure::sectL );
    dEf->setReadOnly( true );
    dEf->setRichName( tr("d<span style=\" vertical-align:sub;\">ef</span>"));
    dEf->setToolTip( tr("Diametro effettivo della vite"));
    addVarToContainer( dEf );
    connect( d, &DoublePlus::valueChanged, this, &TimberConnector::setDEf );
    setDEf();

    dHead = new DoublePlus( dd, "dHead", m_unitMeasure, UnitMeasure::sectL );
    dHead->setReadOnly( false );
    dHead->setRichName( tr("d<span style=\" vertical-align:sub;\">head</span>"));
    dHead->setToolTip( tr("Per viti, diametro della rondella, per chiodi diametro della testa del chiodo"));
    addVarToContainer( dHead );

    lEf = new DoublePlus( dd, "lEf", m_unitMeasure, UnitMeasure::sectL );
    lEf->setReadOnly( false );
    lEf->setRichName( tr("l<span style=\" vertical-align:sub;\">ef</span>"));
    lEf->setToolTip( tr("Per viti, lunghezza della porzione filettata meno un diametro"));
    ...
    alpha->setToolTip( tr("Inclinazione del connettore rispetto alle fibre di legno"));
    addVarToContainer( alpha );

    MyRk = new DoublePlus( 0.0, "MyRk", m_unitMeasure, UnitMeasure::loadM );
    MyRk->setRichName( tr("M<span style=\" vertical-align:sub;\">y,Rk</span>"));
    ...
    connect( MyRk, &DoublePlus::readOnlyChanged, this, &TimberConnector::setMyRk );
    connect( d, &DoublePlus::valueChanged, this, &TimberConnector::setMyRk );
    connect( fuk, &DoublePlus::valueChanged, this, &TimberConnector::setMyRk );
    setMyRk();

    connect( connectorType, &TimberConnectorType::valueChanged, this, &TimberConnector::setVisibleVar );
    setVisibleVar();
}
...
void TimberConnector::setVisibleVar(){
    if( connectorType->valueNormal() == TimberConnector::SmoothRoundNail || \
        ...
        dHead->setRichName( tr("d<span style=\" vertical-align:sub;\">h</span>"));
        dHead->setToolTip( tr("Diametro della testa del chiodo"));
    } else if( connectorType->valueNormal() == TimberConnector::Bolt || \
               connectorType->valueNormal() == TimberConnector::Screw ){
        dHead->setEnabled( true );
        dHead->setRichName( tr("d<span style=\" vertical-align:sub;\">washer</span>"));
        dHead->setToolTip( tr("Diametro della rondella"));
    } else {
...
public:
    TimberConnectorTypePrivate(TimberConnector::ConnectorType v):
        value(v){
        enumList.append( enumVal( TimberConnector::Bolt, "Bolt", tr("Bullone/Spinotto")) );
        enumList.append( enumVal( TimberConnector::Screw, "Screw", tr("Vite")) );
        enumList.append( enumVal( TimberConnector::SmoothRoundNail, "SmoothRoundNail", tr("Chiodo a gambo liscio, rotondo")) );
        enumList.append( enumVal( TimberConnector::SmoothSquareNail, "SmoothSquareNail", tr("Chiodo a gambo liscio, quadrato")) );
        enumList.append( enumVal( TimberConnector::OtherRoundNail, "OtherRoundNail", tr("Chiodo non a gambo liscio, rotondo")) );
        enumList.append( enumVal( TimberConnector::OtherSquareNail, "OtherSquareNail", tr("Chiodo non a gambo liscio, quadrato")) );
    };
...
public:
    TimberPredrilledHolePrivate( TimberConnector::PredrilledHole v):
        value(v){
        enumList.append( enumVal( TimberConnector::WithPredrilledHole, "WithPredrilledHole", tr("Con preforo")) );
        enumList.append( enumVal( TimberConnector::WithoutPredrilledHole, "WithoutPredrilledHole", tr("Senza preforo")) );
    };
