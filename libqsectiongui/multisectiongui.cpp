...
*/
#include "multisectiongui.h"
#include "ui_multisectiongui.h"
...
public:
    MultiSectionGUIPrivate(){
        section = NULL;
        ui = new Ui::MultiSectionGUI;
    }
    ~MultiSectionGUIPrivate(){
        delete ui;
    }
    ...
    Ui::MultiSectionGUI * ui;
    SectionGeometricDataPanel * geometricDataPanel;
    SectionTransformPanel * transformationPanel;
};

MultiSectionGUI::MultiSectionGUI(UnitMeasure* ump, MaterialModel * matModel, QWidget *parent) :
    SectionGUI(ump, matModel, parent),
    m_d( new MultiSectionGUIPrivate() ){

    m_d->ui->setupUi(this);

    m_d->geometricDataPanel = new SectionGeometricDataPanel( m_d->ui->geometricDataTab );
    m_d->ui->geometricDataTab->layout()->addWidget( m_d->geometricDataPanel );
    m_d->transformationPanel = new SectionTransformPanel( m_unitMeasure, m_d->ui->transformationTab );
...
MultiSectionGUI::~MultiSectionGUI() {
    delete m_d;
}

void MultiSectionGUI::setSection(Section * s){
    if( m_d->section ){
    ...
    m_d->transformationPanel->setSection( s );
}

void MultiSectionGUI::addSectionLine(){
...
void MultiSectionGUI::removeSectionLine(){
    QModelIndexList listRows = m_d->ui->sectionLineTableView->selectionModel()->selectedRows();
    if( listRows.size() > 0 ){
...
void MultiSectionGUI::addSectionArc(){
...
void MultiSectionGUI::removeSectionArc(){
    QModelIndexList listRows = m_d->ui->sectionArcTableView->selectionModel()->selectedRows();
    if( listRows.size() > 0 ){
...
Material * MultiSectionGUI::material(){
    if( m_d->section != NULL ){
...
void MultiSectionGUI::setMaterialFromComboBox(){
